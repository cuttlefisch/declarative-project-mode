...
#+begin_src json
{
  ...
  "local-files": [
    {"src": "/path/to/file", "dest": "local-copy"}
  ],
  "symlinks": [
    {"targ": "/path/to/target", "link": "my-link"}
  ],
...
You can embed project specs directly in org files as src blocks:
...
Execute with ~C-c C-c~ on the src block.
...
The ~:dir~ header argument sets the project root directory (defaults to ~default-directory~).
